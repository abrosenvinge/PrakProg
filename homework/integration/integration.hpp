...
namespace pp {
	std::tuple<double,double> integrate(const std::function<double(double)>& f, double a, double b, double acc = 0.000001, double eps = 0.000001);
	...
	std::tuple<double,double> Clenshaw_Curtis_quad(const std::function<double(double)>& f, double a, double b, double acc = 0.0001, double eps = 0.0001);
